 ...
 * to reset.
 ************
 userMode variable sets the mode of the vibrator
 ************
 */
//=======Libraries===============================
#include <Encoder.h>
...
int pressure = 0;
int averagePressure = 25; //Running 25 second average pressure
int rampUp = 10; //Ramp-up time, in seconds, default 30 amended to 10
int userMode = 6;// 1=Half of rampup, 2=Double rampup, 3=Time in seconds (cooldown), 4=Slow Creep (Extends cooldown up to max when edge is reached), 5=More Sensitive (will lower level of sensitivity every edge), 6=The more you clench, the slower the vibrator gets.
int userModeTotal = 6;
...
void loop() {
	...
	Serial.print("motor:"); Serial.print(motorSpeed);       // Motor speed (0-255)
	Serial.print(",");
	Serial.print("pres:"); Serial.print(pressure);          // Raw ADC value (0-4095)
	Serial.print(",");
	Serial.print("avg:"); Serial.print(averagePressure);    // Running average pressure
	Serial.print(",");
	Serial.print("delta:"); Serial.print(pressure - averagePressure); // How far above average (what triggers the switch)
	Serial.print(",");
	Serial.print("limit:"); Serial.print(pressureLimit);    // Sensitivity threshold
	Serial.print(",");
	Serial.print("cooldown:"); Serial.println(minimumcooldown); // println to end the line
  }
}